...
shimmer::menu::menu ( const std::shared_ptr<event_system> event_system )
...
void shimmer::menu::build_from ( const shimmer::config& conf )
{
        for ( auto entry : conf ) {
                ...
                for ( unsigned int i = 0; i < conf_split.size(); i++ ) {
                        ...
                        if ( i < ( conf_split.size() - 1 ) ) {
                                ...
                                _data[curr_path][conf_split[i]] = menu_item ( conf_split[i], entry.second );
                                curr_path += "." + conf_split[i];
                        }
                }
        }
