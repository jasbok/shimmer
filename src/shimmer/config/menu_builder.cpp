...
shimmer::menu_builder::menu_builder ( const std::shared_ptr<event_system>& event_system )
        : _event_system ( event_system )
{}

shimmer::menu_builder::~menu_builder()
{}

shimmer::menu_builder::shimmer_menu_system shimmer::menu_builder::build ( const shimmer::config& conf )
{
...
std::string shimmer::menu_builder::format_title ( const std::string& str )
{
...
shimmer::menu_builder::shimmer_menu_system shimmer::menu_builder::_generate_menu_items ( const shimmer::config& conf )
{
        ...
        for ( auto entry : conf ) {
                ...
                for ( unsigned int i = 0; i < conf_split.size(); i++ ) {
                        ...
                        if ( i < ( conf_split.size() - 1 ) ) {
                                ...
                                items[curr_path][conf_split[i]] = menu_item ( conf_split[i], entry.second );
                                curr_path += "." + conf_split[i];
                        }
                }
        }
