...
void gl_font::operator= ( gl_font && move )
{
...
gl_glyph& gl_font::operator[] ( unsigned char c )
{
    return glyph ( c );
}

void gl_font::font_face ( shimmer::font_face& face )
{
    ...
    for ( unsigned int c = 0; c < range; c++ ) {
        ...
        bool valid_bitmap_and_buffer = g->bitmap.width > 0
                                       && g->bitmap.rows > 0
                                       && g->bitmap.buffer != nullptr;

        if ( valid_bitmap_and_buffer ) {
            glyph
...
gl_glyph& gl_font::glyph ( unsigned char c )
{
    return _glyphs[c < _glyphs.size() ? c : 0];
}

coordinates<int> gl_font::kern ( unsigned char left, unsigned char right )
{
