...
public:
    ...
    std::shared_ptr<shader> create (
        const std::vector<std::string>& vs_shaders,
        const std::vector<std::string>& fs_shaders );

private:
    std::vector<std::string> _read_sources (
    ...
    std::vector<GLuint> _compile_sources (
        const std::vector<std::string>& sources,
        GLuint type );

    GLuint _link_compiled (
        const std::vector<GLuint>& vs,
        const std::vector<GLuint>& fs );

    std::vector<glsl_variable> _read_variables (
        const std::vector<std::vector<std::string>>& sources_vec,
        GLuint program );

    std::shared_ptr<uniform_output> _uniform_output_from (
        const glsl_variable& variable );

    std::vector<std::shared_ptr<uniform_output>> _create_uniform_outputs (
