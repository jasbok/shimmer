...
public:
    ...
    virtual void pixels (
        ...
        dimensions<unsigned int>>& rect ) = 0;

    virtual void* map_buffer() = 0;

    virtual void unmap_buffer() = 0;

    virtual void source_format (
    ...
    virtual void foreground_shaders (
        const std::vector<std::string>& vertex_shaders,
        const std::vector<std::string>& fragment_shaders ) = 0;

    virtual void background_shaders (
        const std::vector<std::string>& vertex_shaders,
        const std::vector<std::string>& fragment_shaders ) = 0;

    virtual void menu_shaders (
        const std::vector<std::string>& vertex_shaders,
        const std::vector<std::string>& fragment_shaders ) = 0;
