...
void video::setup ()
{
    ...
    if ( !_renderer ) {
        _renderer = std::unique_ptr<renderer> (
                        opengl_renderer::create ( _config ) );

        _renderer->source_format (
            _application_resolution,
            _bpp,
            _pixel_format,
            _pixel_type );

        _renderer->background_shaders (
            _config->read_strs ( "renderer.background.vertex" ),
            _config->read_strs ( "renderer.background.fragment" ) );

        _renderer->foreground_shaders (
            _config->read_strs ( "renderer.foreground.vertex" ),
            _config->read_strs ( "renderer.foreground.fragment" ) );

        _renderer->menu_shaders (
            _config->read_strs ( "renderer.menu.vertex" ),
            _config->read_strs ( "renderer.menu.fragment" ) );

        _renderer->font (
            _config->read_str ( "renderer.font.regular" ),
            _config->read_str ( "renderer.font.bold" ),
            _config->read_int ( "renderer.font.size" ) );

        _renderer->font_shaders (
            _config->read_strs ( "renderer.font.vertex" ),
            _config->read_strs ( "renderer.font.fragment" ) );
    }

    if ( _bpp == 0 ) {
        std::cerr << "Warning: Setting up video with BPP value of 0...\n";
    }
...
void video::update ( void* pixels )
{
    _renderer->pixels ( pixels );
    render();
}

void video::update ( void* pixels, const std::vector<rectangle<> >& rects )
{
...
void video::resize ( const dimensions<>& application, const dimensions<>& video )
{
    bool do_resize = false;
    _application_resolution = application;

    bool video_within_min_max_resolution =
        video.w >= 128 &&
        video.w <= _max_resolution.w &&
        video.h >= 128 &&
        video.h <= _max_resolution.h;

    if ( video_within_min_max_resolution ) {
        do_resize = _video_resolution.w != video.w
    ...
    _event_system->resolution_change.emit (
        _application_resolution,
        _video_resolution );
}

void video::resize_application ( const dimensions<>& application )
{
    resize ( application, _video_resolution );
}

void video::resize_video ( const dimensions<>& video )
{
    resize ( _application_resolution, video );
}

void * video::map_buffer()
{
    return _renderer->map_buffer();
}

void video::unmap_buffer()
{
    _renderer->unmap_buffer();
}

void video::bind_source_texture_unit()
{
    _renderer->bind_source_texture_unit();
}

void video::bind_source_fbo()
{
    _renderer->bind_source_fbo();
}

void video::_calculate_aspect_ratio()
{
    if ( _config->is_value ( "renderer.aspect_ratio", "stretch" ) ) {
        _aspect_ratio = { 1.0f, 1.0f };
    } else {
        float ar_app = _application_resolution.w
                       / ( float ) _application_resolution.h;
        ...
        if ( !_config->is_value ( "renderer.aspect_ratio", "original" ) ) {
            auto ar =
                _config->read_floats ( "renderer.aspect_ratio", "\\s*:\\s*" );
